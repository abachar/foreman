...
- R6b — **A tree, one per group** (2026-08-30, from use): each of the three groups renders its own paths as a tree instead of a flat list. A folder whose only child is a folder is **folded into one row** (`src/main/java`), as IntelliJ does. Order inside a level: folders first, then names in `localizedStandardCompare` order (same as `explorer` R2). Everything is expanded, nothing is persisted, and a folder row carries **no action**: staging and discarding stay per file (R7) and per group. The file row itself does not change. No list/tree toggle: a tree of three files reads as three rows.
