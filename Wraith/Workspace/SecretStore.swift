...
///
/// The account of a Postgres password is `wraith.postgres.<host>:<port>/<database>/<user>`
/// (postgres, technical options). A secret never reaches a log, `.wraith/` or an error.
...
nonisolated struct KeychainSecretStore: SecretStore {
    static let service = "dev.crafters.wraith"
