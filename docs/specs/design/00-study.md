...
- R6 — **Typography.** Two families, not three: the **interface font** for the chrome — the system font by default, `theme.interfaceFont` in `config.json` otherwise, at `theme.interfaceFontSize` (13 by default; amended 2026-08-28) — and `ThemeService.editorFont` (the monospaced font from `terminal` R14, **JetBrains Mono 13** by default since 2026-08-28) for code, terminal, diffs and SQL. Three interface sizes only, derived from the body size (`small` = body − 2, `body`, `title` = body + 1) and two weights (`regular`, `medium`). The **markdown preview** has its own scale (2026-08-29): body = `theme.readingFontSize` (**16**) with 4 pt of leading, H1–H6 = × 2 / 1.5 / 1.25 / 1 / 0.875 / 0.85 in `semibold` (`medium` until 2026-08-30), small = × 0.875, code (blocks and inline) = the code font at × 0.85. **Amended 2026-08-30** from the comparison with GitHub ([`01-markdown-vs-github.md`](01-markdown-vs-github.md), M16 16.3–16.4), taking its structure in our own tokens and none of its colours:
  - **Every space is a multiple of the reading size** (`MarkdownMetrics`), as `github-markdown-css` derives all of its from a 16 px base, so raising `readingFontSize` scales the whole layout: between two blocks × 1, above a heading × 1.5 in total, list content at × 2 (a fixed marker column of × 1.5 plus × 0.5), between two items of a list × 0.25, between the blocks of one item × 1, inside a code block × 1, from a quote's bar to its text × 1, a table cell × 0.375 / × 0.8125.
  - **A 1 pt `separator` rule under `h1` and `h2`**, at 0.3 × the heading's own size below it.
  - **Tables get a grid**: a 1 pt `border` around every cell, every second row on `surfaceRaised`, the header in bold.
  - **Inline code takes the `surfaceSunken` ground** of code blocks. Flat: an `AttributedString` run carries a background colour but neither padding nor a corner radius, so GitHub's chip is matched in colour and not in shape.
  - **List markers follow the depth**, as CSS does: disc → circle → square, decimal → lower-roman → lower-alpha, cycling past the third level.
  - **Links are underlined at rest.** GitHub underlines on hover; SwiftUI's `Text` has no per-run hover, and hit-testing the layout by hand was refused as too much code for the least-seen gap (author's call).
  - **Deliberately not taken** (author's call 2026-08-30): no maximum reading width — the preview uses the tab it is given; the horizontal rule stays a 1 pt hairline, not GitHub's 4 px bar; the quote bar stays 3 pt; `h6` stays body-coloured; the markers stay `secondary`; the task checkbox stays an SF Symbol; the code block keeps its own ground. A view asks `ThemeService.font(_:weight:)` / `interfaceFont(_:weight:)`, never a SwiftUI text style.
