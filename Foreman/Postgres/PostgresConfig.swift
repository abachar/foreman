...
/// The `postgres` section of `.foreman/config.json` (postgres R1, R12; config R3, R5).
///
...
nonisolated struct PostgresConfig: Equatable, Sendable {
    ...
    var keychainAccount: String {
        "foreman.postgres.\(host):\(port)/\(database)/\(user)"
    }
    ...
    static func decode(from config: WorkspaceConfig) -> Outcome {
        ...
        guard let section else {
            return .missing("No \"postgres\" section in .foreman/config.json. Example: \(example)")
        }
