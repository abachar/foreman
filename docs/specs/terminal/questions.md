...
- [ ] Copy/paste: `cmd+c` copies the selection, but might an agent want `cmd+c`? No: `ctrl+c` goes to the process, `cmd+c` stays with Wraith (`layout` R25). To be confirmed in use. → **M6 task 6.2**.
