...
- R1 — Repos = `config.repos` (`config` R3), otherwise auto-detection (`.git/` up to depth 2, shared exclusions). The workspace root itself, if it is a repo, is the repo `"."`. A `.git` file (worktree/submodule) is accepted. **Amended 2026-09-09** (issue #5): a `repos` entry starting with `!` **excludes** that repo from tracking — removed from the declared list or from the scan's result (`"!."` excludes the root repo). An exclusion is kept even when the folder is missing; one outside the root is dropped with a warning, like any entry. Exclusions never reach the other consumers of `config.repos` (`agents`, `run`), which keep the declared list only.
- R1b — **Without a repo, no git** (2026-08-29, `layout` R36): the Changes and History panels, their toggles and `cmd+shift+g` / `cmd+shift+h` exist only when R1 finds a repo. The root's `.git` or `config.repos` answers at once; the depth-2 scan answers off the main actor and registers the panels late; a config change re-checks. `git` missing (R28) still shows the panel with its banner: the repo exists, the tool does not.
...
- R6 — Two lists per repo: **Staged** and **Changes** (worktree + untracked), plus **Conflicts** at the top when there are any. Each row: status (`M A D R C U ?`), the file name in bold (the folder in grey until R6b made the tree say it), buttons on hover: stage/unstage, discard, open the file.
- R6b — **A tree, one per group** (2026-08-30, from use): each of the three groups renders its own paths as a tree instead of a flat list. A folder whose only child is a folder is **folded into one row** (`src/main/java`), as IntelliJ does. Order inside a level: folders first, then names in `localizedStandardCompare` order (same as `explorer` R2). Everything is expanded at first sight and a click on a folder row closes it (its chevron and its icon say which way it is); the fold is **not persisted** — reopening the panel opens everything again. A folder row carries **no git action**: staging and discarding stay per file (R7) and per group. The file row itself does not change, except its grey second line: the tree above it already spells the parent folder out, so the line keeps only a rename's origin (`← old/path`). No list/tree toggle: a tree of three files reads as three rows.
