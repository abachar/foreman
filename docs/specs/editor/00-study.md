...
- R11 — v1 grammars (mapped by extension and by file name): java, kotlin, typescript, tsx, javascript, json, yaml, toml, markdown, bash (`.sh`, `.zsh`, `.zshrc`…), swift, html, css, dockerfile (`Dockerfile*`); sql was to arrive with `postgres` (M5, decision 2026-08-26) but its package still does not resolve (decision 2026-08-27, `questions.md`): plain text until then. Unknown extension → plain text.
