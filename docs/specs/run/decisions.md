...
| 2026-08-26 | `env` par commande ou par repo, injecté dans l'environnement du process (remplace « préfixé `K=V` à la commande » du 2026-08-25) ; la commande elle-même n'est jamais modifiée | Préfixe `K=V` dans la ligne ; templating | Wraith possède le process (`terminal` R3) : plus rien à échapper |
| 2026-08-26 | État running/exit code dérivé du PTY (`waitpid`, `terminal` R6) (remplace « OSC 133 » du 2026-08-25) | Shell integration ; process hors terminal | Wraith possède le process : fiable quel que soit le shell ; l'environnement du login shell est conservé par `$SHELL -l -c` |
...
| 2026-08-25 | La palette (fuzzy + UI) devient une capacité du noyau (`PaletteService`), partagée avec le quick open — entériné dans `coding-rules` R5.10 le 2026-08-26 | Composant dupliqué dans chaque plugin | `coding-rules` R4.1/P1 : deux plugins en ont besoin, c'est donc du contrat |
