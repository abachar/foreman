...
Plugin `postgres` : explorer le schéma d'une base et exécuter des requêtes en lecture, sans quitter le workspace. Panneau droit `postgres.schema` (`cmd+shift+d`), panneau bas `postgres.query` (éditeur SQL + grille de résultats, `cmd+shift+q`). Client : PostgresNIO, connexion paresseuse, secrets dans le Keychain.
...
- US4 — Je sélectionne deux lignes de SQL sur cinq : seule la sélection s'exécute.
...
- R5 — État de connexion visible (pastille : déconnecté / connexion / connecté / erreur) ; erreur affichée en bannière avec le message serveur traduit (`PostgresError` Wraith, jamais le type NIO, `coding-rules` §12.4).
...
- R7 — Chargement **par niveau** à l'expansion (`coding-rules` P4/R15.3), via requêtes sur `pg_catalog` (pas `information_schema`, trop lent), une requête par niveau, bornée (timeout 10 s). Aucun rafraîchissement automatique : bouton *Rafraîchir* (par nœud ou global).
...
- R9 — Le panneau `postgres.query` est divisé verticalement : éditeur SQL en haut (redimensionnable), résultats en bas. L'éditeur est une zone de texte monospace propre au plugin, colorée via le `HighlightService` du noyau (grammaire `sql`, `coding-rules` R5.10), avec undo, indentation, `cmd+/` commentaire `--`. Son contenu est persisté dans `state.json`.
- R10 — Exécution (`cmd+enter`, portée panneau) : la **sélection** si elle existe, sinon l'**instruction sous le curseur** (délimitée par `;` hors chaînes/commentaires, split côté client). `cmd+shift+enter` exécute tout le contenu, instruction par instruction, arrêt à la première erreur.
...
- R12 — Toute requête est bornée : `statement_timeout` 30 s (réglable dans la config globale `postgres.statementTimeout`), et les résultats sont lus par **curseur/portal** par pages de 500 lignes ; la grille affiche la première page, *Charger 500 de plus* poursuit, *Tout charger* plafonne à 50 000 lignes avec avertissement. Aucun résultat non borné en mémoire (`coding-rules` §12.4).
- R13 — Annulation : bouton *Stop* / `cmd+.` envoie `pg_cancel_backend` via une connexion d'annulation (protocole cancel), puis ferme la connexion si le serveur ne répond pas en 5 s.
- R14 — Une seule exécution à la fois par fenêtre ; `cmd+enter` pendant une exécution est refusé (bip) et non mis en file.
- R15 — Paramètres : pas de paramètres liés dans l'éditeur libre (c'est du SQL tel quel, comme `psql`). En revanche, tout SQL **généré par Wraith** (R8, R7, R12) échappe les identifiants (`quote_ident` côté client) et lie ses valeurs (`coding-rules` §12.4).
...
- R17 — Barre d'état : `N lignes (page 1/…) · 42 ms · user@base`. Une instruction sans résultat (`UPDATE`, `CREATE`) affiche `OK · 3 lignes affectées`. Plusieurs instructions (R10) → un onglet de résultat par instruction, le dernier actif.
...
- **Client** : PostgresNIO (SPM, épinglé `.upToNextMinor`) dans `PluginPostgres/Services/PostgresClient` (`actor`), implémentant `DatabaseService` (protocole du plugin : `connect`, `query(sql, pageSize) -> AsyncStream<ResultPage>`, `cancel`, `close`). Types traduits à la frontière en `QueryResult { columns: [ColumnInfo], rows: [[QueryValue]] }`, `QueryValue` enum (`null`, `text`, `int`, `double`, `bool`, `date`, `json`, `bytes`, `raw(String)`).
- **Curseur** : `DECLARE … CURSOR` explicite est refusé par une session autocommit sans transaction ; on utilise le **protocole étendu avec portal + `Execute(maxRows)`** que PostgresNIO expose via son streaming de lignes, en lisant par lots et en suspendant la consommation entre pages. À valider par prototype au découpage de M5 ; repli : `LIMIT/OFFSET` injectés uniquement sur les requêtes générées par Wraith, et `LIMIT` conseillé pour l'éditeur libre.
- **Annulation** : PostgresNIO supporte l'annulation de tâche (`Task.cancel` → cancel request) ; à vérifier sur la version épinglée.
- **Secrets** : `SecretStore` (`coding-rules` §12.7) avec clé `wraith.postgres.<host>:<port>/<database>/<user>` ; double `InMemorySecretStore` en tests.
- **Split de statements** : tokenizer minimal (chaînes `'…'`, `$$…$$`, identifiants `"…"`, commentaires `--`/`/* */`) ; testé.
- **Tests** : split de statements (R10), formatage des valeurs et export CSV/JSON (R16, R18), fusion globale/workspace de la config (R1), lecture `.pgpass` (R3), FIFO/épinglage de l'historique (R20), `FakeDatabaseService` pour la logique des panneaux.
