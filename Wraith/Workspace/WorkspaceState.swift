...
/// The persisted UI state of a workspace, `<root>/.wraith/state.json` (config R8, R9).
///
...
nonisolated struct WorkspaceState: Sendable, Equatable {
    ...
    static func file(under root: URL) -> URL {
        root.appending(components: ".wraith", "state.json")
    }
    ...
    ///
    /// `.wraith/` is created on the first write (config R1).
    @concurrent
