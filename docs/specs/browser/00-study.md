...
- R7 — `isInspectable`: Safari's Web Inspector through *Inspect Element* (context menu), `cmd+opt+i` (`browser.inspect`, scope `tab(browser.page)`) or Safari's *Develop* menu. Its tab set is the inspector's own setting (right-click on its tab bar; WebKit persists it) — there is no API for the host to restrict it.
...
- R9 — Shortcuts (scope `tab(browser.page)`): `cmd+shift+r` reload (`cmd+r` stays the `run` palette), `cmd+[` / `cmd+]` back / forward, `cmd+opt+i` inspector, `cmd+e` sends the URL to the active agent (`agents` R10b: the URL as plain text). Zoom is WebKit's own `cmd+=` / `cmd+-` (native, no registration).
