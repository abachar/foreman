...
| 0.3 | **`Workspace` : config** : lecture `.wraith/config.json` + `~/.config/wraith/config.json`, précédence, sections par feature (`config.section("x")` → `Decodable`), erreur avec ligne, clé `password` ignorée avec avertissement, `repos` absents ignorés | config R1–R5, R7, R11 | `JSONSerialization` (parse, fusion, `NSJSONSerializationErrorIndex` → ligne), `JSONDecoder` par section | précédence défauts < global < workspace ; JSON invalide → dernière config valide + erreur ligne/message ; `password` ignoré ; section absente → vide | M | 🟢 | |
...
- 0.11 : `cmd+t` reste libre (retiré avec l'onglet shell) — l'assigner à l'accueil (« nouvel onglet » ouvre l'écran d'accueil dans un nouveau groupe ?) ou le laisser libre jusqu'à M1.
