...
Wraith is a native macOS app built around an embedded [Ghostty](https://github.com/ghostty-org/ghostty) terminal (libghostty). The terminal is the core; everything else — file explorer, git panels, editor, Postgres browser — is a plugin that attaches panels around it.

- **Repo**: `abachar/wraith`
- **Bundle ID**: `dev.crafters.wraith`
- **CLI**: `wraith [path]` opens a workspace
- **Target**: macOS only (Apple Silicon first), Swift / SwiftUI, Metal rendering
...
```bash
git clone https://github.com/abachar/wraith.git
cd wraith
# open in Xcode or build with SPM
swift build
```
