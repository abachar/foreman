...
- R28 — Errors translated into `GitError` (`notARepo`, `commandFailed(stderr)`, `needsInteraction`, `timeout`, `conflict`, `gitNotFound`); `git` not found → the feature shows a single banner and stays inert (nothing breaks the opening, `architecture.md`).
