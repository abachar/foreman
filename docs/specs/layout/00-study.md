...
- R37 — **The menu bar is the Mac's, filled from the `ShortcutRegistry`** (2026-08-30, from use; the first wording — one submenu per feature namespace — was tried the same day and dropped: it read as `Layout / Editor / Explorer / Postgres / Git / Terminal / Browser / Run` and overflowed the bar). The bar is `File / Edit / View / Tools / Run`, before the `Window` and `Help` macOS owns; `File`, `Edit` and `View` already exist, so Foreman's entries are **appended** to them under a separator, never a second menu of the same name. The map:
  ...
  | Tools | Agents ▸ (one per `config.agents`) · Send to Agent · Git ▸ · Postgres ▸ · Browser ▸ |
  | Run | Run Command… · Stop Command · Clear Scrollback |

  Nothing is declared twice: the map names action ids and everything else — title, shortcut, what the action does — comes from the registry (R22). **Having a shortcut is not a reason to be in a menu, and having none is not a reason to be left out**: an agent has no default shortcut and belongs there. An action the registry holds once per scope — the four `…sendToAgent` of `agents` R10b — is **one** entry, which the scope in force answers for. An action the map names but no feature registered is dropped, submenus and separators left empty with it: a window without a repo shows no *Git* submenu. The commands of `config.json` stay in the ▶ Run button, not in the menu. Because the actions belong to a window and the bar belongs to the app, the bar is rebuilt when the key window changes.
- R38 — **A double click opens an untitled tab** (2026-08-30, from use): on the **empty part of the tab bar** and on the **home screen** of an empty group (R33), a double click opens an untitled tab (`editor` R34) **in that group** — the group takes the focus first, so the tab lands where it was asked for and not in whichever group was active. The layout does not know what an untitled tab is: the feature that owns them registers what to open (`LayoutManager.onNewTab`), exactly as the home entries come from the features (R33); with nothing registered the double click does nothing. The empty part of the bar is a view **beside** the tabs, not a layer under them, so a double click on a tab never reaches it; on the home screen the rows are buttons and keep their single click (R33). Third way in: the *File ▸ New File* entry of the menu bar (R37), `cmd+n`. The explorer's *New File* is unchanged (`explorer` R16): a right click there designates a folder, so the name is asked up front.
