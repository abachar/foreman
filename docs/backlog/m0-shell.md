...
|---|---|---|---|---|---|---|---|
| 0.1 | **Squelette** : `Package.swift` (une target `Wraith`, une `WraithTests`), `.swift-format`, `App/WraithApp.swift`, fenêtre vide, CI `swift build` + `swift test` + lint | — | SwiftPM, SwiftUI `App` | aucun | S | ⚪ | |
...
- `swift test` couvre toutes les lignes « Tests » ci-dessus ; lint propre ; aucune dépendance SPM ajoutée.
