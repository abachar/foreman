...
(M0 → M6) et chaque dépendance à venir (libghostty, libgit2, PostgresNIO, tree-sitter…) s'y insèrent sans renégociation.
...
- **P3 — Le plus simple qui marche.** Un seul utilisateur, pas de compatibilité ascendante à garantir en v1 (`00-product`). On n'écrit pas d'abstraction pour un besoin hypothétique — sauf celles imposées par P2, qui protègent d'un remplacement *déjà prévu* (libghostty ↔ SwiftTerm, libgit2 ↔ binaire `git`).
...
| Textes affichés à l'utilisateur | anglais (utilisateur unique, cf. `00-product`) |
...
│   ├── WraithGhostty/      # adaptateur libghostty (seul module qui importe le C)
│   ├── PluginExplorer/
│   ├── PluginEditor/
│   ├── PluginGit/
│   ├── PluginPostgres/
│   └── PluginRun/
├── Vendor/
│   └── libghostty/         # dépendance vendorisée, pinnée à un commit
├── cli/
│   └── wraith              # script shell
├── Tests/
│   ├── WraithAppTests/
...
- **R4.1 — Sens des dépendances** (aucune flèche inverse, aucune exception) :
  ...
  ```
  PluginX ──▶ WraithKit ◀── WraithApp ──▶ WraithGhostty ──▶ Vendor/libghostty
  ```
...
- **R5.6 — Le noyau ne connaît pas les schémas de config des plugins.** Chaque plugin lit sa section via `config.section("<id>")` et la décode lui-même (`01-config` R5).
...
- **R6.9** — Un `TODO` doit référencer une spec : `// TODO(05-editor): reload strategy on external change`. Sans référence, il n'est pas commité.
...
- **R8.9** — Une erreur que l'utilisateur doit connaître se voit dans l'UI (bannière du panneau concerné) : un log seul ne suffit pas. Une erreur de config affiche le fichier, la ligne et le message (`01-config` R7).
...
- **R10.3** — La racine du workspace est la seule base : un chemin interne est stocké **relatif** à la racine, absolu sinon (`01-config` R10). La normalisation est faite par `WorkspaceService`, pas dans chaque plugin.
...
- **R10.7** — L'échec d'une écriture (disque plein, lecture seule) est dégradé et signalé une fois, jamais fatal (`01-config`, cas limites).
...
- **R10.9** — Tout format persisté porte un numéro de version de schéma ; un état de version inconnue est ignoré et sauvegardé en `.bak` (`01-config` R9).

---

## 11. Dépendances : règle générale

- **R11.1 — Adaptateur obligatoire.** Une librairie tierce est importée dans **un seul module**, l'adaptateur. Le reste du code parle à un protocole Wraith. Concrètement : `import PostgresNIO` n'apparaît que dans `PluginPostgres/Services/`, `import SwiftTreeSitter` que dans le service de highlighting, le C de ghostty que dans `WraithGhostty`.
...
|---|---|---|---|---|
| libghostty (C, vendorisé) | émulation VT + rendu Metal | `WraithGhostty` | `TerminalSurface`, `TerminalService` | SwiftTerm |
| PTY système (`forkpty`, POSIX) | processus shell | `WraithApp/Services` | `PTYSession` | — |
...
| SwiftTreeSitter + grammaires | highlighting | `PluginEditor/Services` | `HighlightSpan` | affichage brut sans couleur |
...
### 12.1 libghostty — `03-terminal`

- Le C n'existe **que** dans `WraithGhostty`. Aucun `import CGhostty`, aucun pointeur, aucun `ghostty_*` ailleurs dans le dépôt.
- L'API exposée est un protocole `TerminalSurface` (créer, redimensionner, écrire des octets, lire les événements, focus, thème) qui doit rester implémentable par SwiftTerm ; toute méthode impossible à couvrir par le repli est un signal d'alarme, à trancher dans `03-terminal`.
- Version épinglée à un commit précis, écrite dans `Vendor/libghostty/README.md` avec la date et la raison. Toute montée de version est un commit dédié (R11.5) qui rejoue le scénario : ouvrir, redimensionner, `vim`, `htop`, `exit`.
...
- Cadence : ce qui vient du PTY est agrégé/débouncé avant de toucher l'UI ; pas un événement SwiftUI par octet.

### 12.2 PTY — `03-terminal`

- Le PTY est encapsulé dans un `actor` (`PTYSession`) : ouverture, `forkpty`, taille (`TIOCSWINSZ`), lecture, écriture, terminaison.
- Fermeture toujours déterministe : à la fermeture d'un onglet, le process reçoit `SIGHUP`, les descripteurs sont fermés, l'`actor` est libéré. Pas de descripteur qui fuit, pas de zombie.
- L'environnement du shell est construit explicitement (`TERM`, `cwd`, variables de la config), jamais hérité tel quel sans revue.
- Aucune commande n'est construite par concaténation de chaîne non échappée ([§16](#16-sécurité)).

### 12.3 git — `06-git`

- Le choix libgit2 vs binaire `git` est ouvert (`06-git`) : **le code doit rester indifférent**. Un seul protocole `GitService` ; l'implémentation est un détail interne du plugin.
...
### 12.4 PostgresNIO — `07-postgres`

- Connexion **paresseuse** (P4) : rien avant l'ouverture effective du panneau. Fermeture à la désactivation.
- Aucun mot de passe en config ni en mémoire au-delà du besoin : lecture Keychain à la connexion (`01-config` R3/R11).
...
### 12.5 tree-sitter — `05-editor`

- Les grammaires sont des dépendances épinglées ; la liste des langages v1 est décidée dans `05-editor`. Ajouter une grammaire = ajouter une ligne de mapping, pas du code.
...
- Fichiers trop gros ou binaires : détectés **avant** le parsing, seuil défini dans `05-editor`.
...
S'applique à libghostty, libgit2 et aux appels POSIX.
...
- **R15.4 — Les rafales sont amorties à la source** : FSEvents ~300 ms, `state.json` ~1 s (`01-config` R8), sortie PTY par lots.
...
- **R16.1** — Aucun secret dans le dépôt, dans `.wraith/`, dans les logs, dans les messages d'erreur. Les secrets vivent uniquement dans le Keychain (`01-config`).
- **R16.2** — Une clé `password` détectée dans `config.json` est ignorée avec avertissement (`01-config` R11) : on ne la lit pas « juste au cas où ».
- **R16.3** — Aucune exécution de commande construite par interpolation dans un shell. `Process` avec `arguments: [String]`, exécutable résolu explicitement. Les commandes de la config (`08-run`) sont envoyées à un terminal de l'utilisateur, ce qui est leur nature — mais jamais recomposées avec des valeurs venant d'un autre fichier ou d'une sortie de programme.
...
- **R16.5** — Aucun accès réseau non demandé par l'utilisateur : pas de télémétrie, pas de check de mise à jour, pas de chargement de ressource distante (`00-product` R10).
...
- **R17.4** — Le corps du commit dit **pourquoi** ; le diff dit quoi. On y référence la spec concernée (`docs/specs/02-layout`).
...
- **R18.2** — Une question ouverte tranchée quitte la liste `Questions ouvertes` du `README.md` du domaine et devient une ligne datée dans `01-decisions.md` (date, décision, alternatives rejetées, raison).
