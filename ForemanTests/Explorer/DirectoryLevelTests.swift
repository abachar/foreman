...
struct DirectoryLevelTests {
    ...
    @Test func hidesGitAndStateButShowsDotfiles() async throws {
        try make(
            directories: [".git", ".github", ".foreman"],
            files: [".env", ".DS_Store", ".foreman/state.json", ".foreman/config.json"])
        let foreman = try await DirectoryLevel.read(".foreman", root: root, rootIsHome: false)
        #expect(foreman.nodes.map(\.relativePath) == [".foreman/config.json"])
        let level = try await read()
        #expect(level.nodes.map(\.name) == [".foreman", ".github", ".env"])
    }
