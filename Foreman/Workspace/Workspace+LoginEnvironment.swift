...
extension Workspace {
    ...
    private static func resolveLoginEnvironment() async -> [String: String] {
        ...
        let data = await PipeIO.readToEnd(reader)
    ...
    /// Parses the `env -0` output after the sentinel — `KEY=VALUE` records separated by NUL,
    /// values allowed to hold newlines.
    ///
    /// Without the sentinel nothing is trusted and the result is empty.
