...
| 0.12 | **Barre d'outils** : `NSToolbar` + délégué dans `Layout/`, enregistrement d'éléments par les features (`id`, titre, icône SF Symbol, placement, action ou menu, badge), `cmd+opt+t` persisté ; en M0 : vide (validée avec un élément de démonstration retiré ensuite) | layout R30–R32 | `NSToolbar`, `NSToolbarItem`, `NSMenu` | id dupliqué refusé ; ordre leading/trailing | M | 🟢 | |
...
| 0.14 | **Onglet de démonstration** `demo.hello` : un `CenterTabDescriptor` minimal (vue texte, payload = titre) pour exercer onglets, splits, déplacement, persistance de bout en bout ; avec lui, les trois panneaux de démonstration (`cmd+shift+1/2/3`) et l'élément de toolbar (`Wraith/Demo/`) ; supprimé au début de M1 | — | — | aucun | S | 🟢 | |
...
- `cmd+d` / `cmd+shift+d` splittent, `cmd+w` ferme, `cmd+opt+←→↑↓` naviguent, `cmd+opt+shift+←→↑↓` déplacent un onglet `demo.hello`.
