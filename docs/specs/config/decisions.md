...
| 2026-08-26 | **Pas de configuration globale** : une seule source, `<root>/.wraith/config.json` (annule la décision « config globale dans `~/.config/wraith/config.json` » et la fusion global/workspace du même jour) | Config globale XDG + fusion par section | Une source, un fichier à côté du projet : moins de code, pas de question « d'où vient cette valeur » |
| 2026-08-26 | Surveillance de `config.json` via **AsyncFileMonitor** (`FolderContentMonitor` multicast sur FSEvents) ; `FSWatchService` ne garde que le filtrage par chemin et les lots débouncés | Wrapper FSEvents maison (callback C, cycle de vie) ; **swift-configuration** (Apple) écartée : rechargement par *polling* (interdit par `architecture`), lecture par clé typée sans section `Decodable` (R5), pas de ligne d'erreur (R7), conçue pour serveurs multi-sources | `architecture` P2 : la lib fait le travail risqué (le cast du callback FSEvents a coûté un crash) ; ce qui reste est le contrat Wraith |
