...
- R4 — **No chrome (amended 2026-08-29)**: the page fills the island; no address, no back/forward, no reload button. Navigation is the page's links and the shortcuts (R9). *(Until then: a bar with back, forward, reload and the URL.)*
- R5 — Allowed: `http`, `https`, and the config's host; `file:`, `javascript:` and custom schemes are refused with a banner. Certificate errors: WebKit's page, never bypassed. Links with a non-web scheme (`mailto:`, `vscode:`) go to the system, **amended 2026-08-30**: only from a click (or a form submission) in the **main frame**, and only after a confirmation naming the scheme and the URL — a redirect, a script or an `iframe` gets the banner instead. `blob:` loads (the page's own content); a top-level `data:` navigation is refused (**2026-08-30**). `window.open` / `target=_blank` navigate **in the tab** (a single tab, R1). Downloads are refused with a banner.
...
- R8 — JavaScript dialogs (`alert`, `confirm`, `prompt`) as sheets (`NSAlert`), and `<input type="file">` as an `NSOpenPanel` sheet honouring `allowsMultipleSelection` and `webkitdirectory` (**2026-08-30**: it did nothing before). Every sheet belongs to the window the tab lives in, never to `NSApp.keyWindow`; with no window the dialog is dismissed rather than run app-modally. `beforeunload` is not asked when the tab closes (a private session has nothing to lose).
...
- R11 — **Viewports (2026-08-29)**: a floating switcher at the top right of the page — *iPhone* (393 × 852 pt), *iPad* (820 × 1180 pt), *Desktop* (the tab's full size). A device size lays the page out at that width and height, centred on the sunken ground, clipped to the tab when it is smaller; the web view is the same (no reload, the session stays). Not persisted: a tab opens in *Desktop*. Only the layout size changes: no user agent, no touch events, no device frame.
