...
| 2026-08-26 | L'agent vit sur une surface terminal (le seul usage du terminal avec `run`, `product` R4) ; état dérivé du PTY (`terminal` R6) ; aucune intégration profonde en v1 | Pont agent ↔ workspace (sélection, diff auto) | Livrer d'abord le lancement ; explorer/git se rafraîchissent déjà par FSEvents |
