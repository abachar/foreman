...
- R21 — **Splits stay inside the centre island** (author, 2026-08-27): splitting a group never creates a sibling island next to the panels. The centre zone is one island; its groups are laid out inside it, each with `islandRadius` corners and separated by `gutter`, so that the panels keep one neighbour and the split reads as a subdivision of the centre. Mockup `08.jpeg` shows the rejected rendering (two islands of the same rank as the Explorer).
